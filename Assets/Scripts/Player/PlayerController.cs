	...
	void Update () 
	{
		if(Global.Instance.IsPlaying)
		{
			...
			if(ColaBottleParticles != null && CurrentPickup == PickupType.ColaBottle)
			{
				...
				ColaBottleParticles.renderer.material.mainTexture = transform.FindChild("Sprite").renderer.material.mainTexture;;
			}
			...
			transform.rotation = Quaternion.Euler(0, 0, -rigidbody2D.velocity.x * 2);
